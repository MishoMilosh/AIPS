...
public class Competition
{
    public static void competition(SLL<Competitor> prev, SLL<Competitor> current) {
        // to do
        SLLNode<Competitor> previous = prev.getFirst();
        double sumaPoeni = 0;
        while(previous!=null){
            sumaPoeni += previous.element.getPoints();
            previous = previous.succ;
        }
        double prosek = sumaPoeni/prev.length();
        previous = current.getFirst();
        while(previous!=null){
            if(previous.element.getPoints()<prosek){
                current.delete(previous);
            }
            previous = previous.succ;
        }
    }
