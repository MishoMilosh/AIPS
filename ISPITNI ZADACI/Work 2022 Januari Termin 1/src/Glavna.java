...
public class Glavna
{
    ...
    {
        SLLNode<Task> NajbitanToDo = toDo.getFirst();
        SLLNode<Task> pom1 = null;
        SLLNode<Task> pom2 = null;
        int max = 0;
        while(NajbitanToDo!=null){
            int maxImportance = 0;
            maxImportance = 2 * NajbitanToDo.element.getHours() * NajbitanToDo.element.getPriority();
            if(maxImportance > max){
                max = maxImportance;
                pom1 = NajbitanToDo;
                pom2 = pom1;
            }
            NajbitanToDo = NajbitanToDo.succ;
        }
        toDo.delete(pom1);
        inProgress.insertFirst(pom2.element);
        SLLNode<Task> NajnebitanInProgress = inProgress.getFirst();
        SLLNode<Task> pom3 = null;
        SLLNode<Task> pom4 = null;
        int min = 9999;
        while(NajnebitanInProgress!=null){
            int minImportance = 0;
            minImportance = 2 * NajnebitanInProgress.element.getHours() * NajnebitanInProgress.element.getPriority();
            if(minImportance<min){
                min = minImportance;
                pom3 = NajnebitanInProgress;
                pom4 = pom3;
            }
            NajnebitanInProgress = NajnebitanInProgress.succ;
        }
